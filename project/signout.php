...
This page signs user out of a project.
...
if (isset($_GET['id'])) {
    if((isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] === true)){
                ...
                if($stmt = mysqli_prepare($c, $sql)){
                    ...
                    if(mysqli_stmt_execute($stmt)){
                        ...
                        if(mysqli_stmt_num_rows($stmt) == 1){
                                ...
                                $sql = "DELETE FROM user_has_project WHERE user_id = ? AND project_id = ? AND role = ?";
                                ...
                                if($stmt = mysqli_prepare($c, $sql)){
                                    ...
                                    if(!mysqli_stmt_execute($stmt)){ 
                                        ...
                                        $message = "<h2>Success</h2> <p>You have successfully signed out of project " . $project_name . ".</p>";
                                    }
                                }
                        } else{
                            $message = '<h2>Error</h2> <p>You are not member of project "'. $project_name .'".</p>';
                        }
                    } else{
                        echo "It seems like we are having issues. Please try again later!";
                    }
        ...
        $message = "<h2>Error</h2> <p>You need to be logged in to sign out of a project. <a href=\"../students/signup.php\">Click here to register an account</a>.</p>";
    }
} else {
    // Fallback behaviour
    header("location: ../index.php");
...
<html lang="en">
  <head>
    ...
    <title>Community Connectors - Project Signout</title>
  </head>
  <body>
